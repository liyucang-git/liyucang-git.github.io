...
在单页面前端项目的开发中，随着业务的不断累积，业务前端代码量往往会达到几万行～几十万行，业务复杂度类比Excel等桌面应用。管理好10万行级甚至百万行级代码的前端应用，已经成为了越来越多前端团队的核心挑战之一。
...
在ajax 拦截器中(config/interceptors/axios.js)：
...
* 不允许在一个模块内部直接调用其他模块的方法（写操作、变更其他模块的state）
