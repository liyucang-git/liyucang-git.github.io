...
vue-i18n是一个针对于vue的国际化插件，本文基于vue-i18n实现vue项目的国际化，并对其中常见的疼点给出相应的解决办法。
...
接着来看问题2和问题3，都是因为需要用字母做key才导致的，那我们是不是可以用中文来做key：
...
如果对于同一个中文，给出的翻译是一致的，那就可以去除前缀的限制。于是我们查阅了项目中对于同一个中文给出的翻译情况，发现对于我们的项目，同一个中文的英文翻译基本是相同的，偶尔不同也是大小写的区别或者是同义不同词的翻译。

所以我们以中文为key，但由于中文中可能会存在标点符号，而标点符号在yml文件中可能会引起格式不正确，这里我们对中文做一次hash再当作key：
...
同时，因为是中文的hash做key，所以对于不同的中文会对应不同的翻译，这样我们就可以在需要使用同一个中文对应不同的翻译时，为其中某个中文添加额外信息：
...
最后我们来看第一个问题，每次都需要手动添加新的中文内容到翻译文件吗？

不，我们拒绝这份重复而繁重的工作。

由于我们在项目中采用我们封装过的`$spt()`来包裹中文，我们可以轻易的找出项目中所有的中文，那我们就可以写脚本帮我们完成这个工作了，代码如下：
...
// const ymlData2 = YAML.parse(fs.readFileSync(path.resolve('./system/i18n/oldData.yml'), 'utf-8'));
...
function getValue(hash, key) {
  ...
  // const obj2 = ymlData2[key] || {};

  return {
    enStr: obj1.en || ''
    // enStr: obj1.en || obj2.en || ''
  };
}
...
function transferFile(filePath) {
  ...
  while (result) {
    ...
    const noSymbolStr = removeSymbol(zhCnStr);
    const { enStr } = getValue(hashStr, `${fileKey}.${noSymbolStr}`);
    ...
    console.log(numSinger, numTwoSinger);
  }
}
