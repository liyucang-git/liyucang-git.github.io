...
exports.getLastGitCommit = () => {
  ...
  const commitMsg = exec('git show -s').toString().trim();
  const rev = fs.readFileSync('.git/HEAD').toString();

  // 在一个commit提交点上，没有对应的分支
  if (rev.indexOf(':') === -1) {
    return {
      commitId: rev,
      commitMsg
  ...
  const head = ('.git/' + rev.substring(5)).trim();
  const commitId = fs.readFileSync(head).toString().trim();
  return {
...
```
// 全局调试工具，集中挂载在window.test上
const test = {};

// eslint-disable-next-line
test.getCurrentVersion = () => __VERSION__;

// 禁止随意在window全局放置变量或方法
window && (window.test = test);
```
