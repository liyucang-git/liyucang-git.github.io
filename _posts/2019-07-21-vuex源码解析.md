...
Vuex 是专为 Vue 开发的统一状态管理工具。当我们的项目不是很复杂时，一些交互可以通过**全局事件总线解决，但是这种观察者模式有些弊端**，开发时可能没什么感觉，但是当项目变得复杂，维护时往往会摸不着头脑，如果是后来加入的伙伴更会觉得很无奈。这时候可以采用 Vuex 方案，它可以使得我们的项目的**数据流变得更加清晰**。本文将会分析 Vuex 的整个实现思路，当是自己读完源码的一个总结。
