...
* queue数组里是因为数据变动触发更新的watch对象，flushSchedulerQueue函数依次执行queue数组中watch对象的`run()`方法。在flushSchedulerQueue函数执行结束后，数据的变动才能再次触发新的flushSchedulerQueue函数，而函数执行结束之前，数据变动产生的watch对象会push到当前到queue数组中。
...
3. `\w` 元字符用于查找单词字符。单词字符包括：a-z、A-Z、0-9，以及下划线, 包含 _ (下划线) 字符
